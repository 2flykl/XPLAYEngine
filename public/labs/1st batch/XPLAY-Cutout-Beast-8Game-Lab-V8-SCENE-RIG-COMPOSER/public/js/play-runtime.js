...
function init(){
 ...
 S.p={x:worldW*.08,y:groundYAt(worldW*.08),hp:100,energy:100,score:0,state:0,vx:0,vy:0,cool:0};
 S.enemies=[];
 const spawnList=M.environment?.spawnPoints || [];
 if(spawnList.length){
   spawnList.forEach((sp,i)=>S.enemies.push({x:sp.x,y:groundYAt(sp.x),hp:70,actor:i%Math.max(1,M.opponents.length),state:0,cool:0}));
 } else {
   for(let i=0;i<4;i++){const ex=worldW*(.36+.14*(i%4));S.enemies.push({x:ex,y:groundYAt(ex),hp:70,actor:i%Math.max(1,M.opponents.length),state:0,cool:0});}
 }
...
function worldToScreen(x,y){return {x:x-camera.x,y:(y/S.worldH)*c.height-camera.y}}

function groundConfig(){
  if(M.sceneRig?.ground)return {baselineRatio:M.sceneRig.ground.playerBaselineRatio,depthMinRatio:M.sceneRig.ground.farYRatio,depthMaxRatio:M.sceneRig.ground.nearYRatio,clampActorsToGround:true};
  return M.environment?.grounding || {
    baselineRatio:0.82,
    depthMinRatio:0.72,
    depthMaxRatio:0.86,
...
function groundYAt(wx, desiredY=null){
  const g=groundConfig();
  const base=S.worldH*g.baselineRatio;
  const min=S.worldH*g.depthMinRatio;
  const max=S.worldH*g.depthMaxRatio;
  if(M.runtime==='racing') return Math.max(min,Math.min(max,desiredY ?? base));
  if(M.runtime==='rhythm') return base;
  if(M.runtime==='brawler'){
    // shallow beat-em-up lane: characters can move in depth, but their feet remain on the floor band.
    return Math.max(min,Math.min(max,desiredY ?? base));
  }
  if(M.runtime==='shooter' || M.runtime==='rpg' || M.runtime==='lifesim' || M.runtime==='horror' || M.runtime==='adventure'){
    return Math.max(min,Math.min(max,desiredY ?? base));
  }
  return base;
}

function snapActorGround(actor){
  actor.y=groundYAt(actor.x,actor.y);
}
...
function updateCamera(dt){
 ...
 camera.y += (targetY-camera.y)*Math.min(1,dt*6.0);
}
...
function drawWorld(){
 ...
 for(let i=0;i<envFar.length;i++){
   ...
   ctx.globalAlpha=.72;
   ctx.drawImage(envFar[i],x,0,drawW,H*1.05);
 }
 ctx.globalAlpha=1;
 ...
 for(let i=0;i<envChunks.length;i++){
   ...
   ctx.drawImage(envChunks[i],x,0,chunkW,H);
 }
...
function drawActor(im,actor,wx,wy,state=0,scale=.23,flip=false){
 const p=worldToScreen(wx,wy),r=frameRect(actor,state%actor.map.states.length);const role=actor.role||'enemy',rs=M.sceneRig?.scale;let base=scale;if(rs)base=role==='player'?rs.playerHeightRatio:role==='npc'?rs.npcHeightRatio:role==='vehicle'?rs.playerHeightRatio:rs.enemyHeightRatio;const gc=groundConfig(),d=gc.depthMaxRatio===gc.depthMinRatio?1:Math.max(0,Math.min(1,(wy/S.worldH-gc.depthMinRatio)/(gc.depthMaxRatio-gc.depthMinRatio))),ds=M.sceneRig?(M.sceneRig.scale.minDepthScale+(M.sceneRig.scale.maxDepthScale-M.sceneRig.scale.minDepthScale)*d):1;const size=Math.min(c.width,c.height)*base*ds;
...
function hud(t){
 ...
 const worldInfo=M.environment?` · WORLD ${Math.floor(camera.x)}/${M.environment.worldWidth} · GROUNDED`:' · STATIC FALLBACK';
...
function movePlayer(dt,speed=320){
 ...
 const g=groundConfig();
 p.y=Math.max(S.worldH*g.depthMinRatio,Math.min(S.worldH*g.depthMaxRatio,p.y));
}
...
function brawler(dt){
 ...
 if(!S.enemies.length){S.tasks++;S.objective=`Advance to zone ${Math.min(3,S.tasks+1)}`;const nx=Math.min(S.worldW-250,p.x+900);for(let i=0;i<4;i++){const ex=nx+i*90;S.enemies.push({x:ex,y:groundYAt(ex,S.worldH*(.76+.03*(i%2))),hp:75+S.tasks*10,actor:i%Math.max(1,M.opponents.length),state:0,cool:0})}}
...
function shooter(dt){
 ...
 S.enemies=S.enemies.filter(e=>e.hp>0);
 if(!S.enemies.length&&S.tasks<3){S.tasks++;const x=Math.min(S.worldW-500,p.x+1000);for(let i=0;i<4+S.tasks;i++){const ex=x+i*110;S.enemies.push({x:ex,y:groundYAt(ex,S.worldH*(.74+.025*(i%3))),hp:75,actor:i%Math.max(1,M.opponents.length),state:0,cool:0});}S.objective=S.tasks===3?'Reach extraction':'Push to next street sector'}
...
function lifesim(dt){
 ...
 M.opponents.forEach((a,i)=>{const ex=S.worldW*(.35+.24*(i%2));drawActor(oppImgs[i],a,ex,groundYAt(ex,S.worldH*(.76+.04*(i%2))),0,.17,true)});
}
...
function horror(dt){
 ...
 const stalk=M.opponents[0];if(stalk&&oppImgs[0]){const ex=Math.min(S.worldW-200,p.x+700-Math.min(500,S.fear*8)),ey=groundYAt(ex);drawActor(oppImgs[0],stalk,ex,ey,Math.min(3,Math.floor(S.fear/12)),.18,true);if(Math.abs(p.x-ex)<120)p.hp-=10*dt}
...
function rpg(dt){
 ...
 S.enemies=S.enemies.filter(e=>e.hp>0);
 if(!S.enemies.length&&S.tasks<3){S.tasks++;const x=Math.min(S.worldW-500,p.x+900);for(let i=0;i<3+S.tasks;i++){const ex=x+i*100;S.enemies.push({x:ex,y:groundYAt(ex,S.worldH*(.74+.03*(i%2))),hp:80+S.tasks*30,actor:0,state:0,cool:0});}S.objective=S.tasks===3?'Defeat guardian':'Reach next crystal zone'}
...
function racing(dt){
 ...
 const g=groundConfig();p.y=Math.max(S.worldH*g.depthMinRatio,Math.min(S.worldH*g.depthMaxRatio,p.y));
 ...
 M.opponents.forEach((a,i)=>{const ex=p.x+520+i*260;drawActor(oppImgs[i],a,ex,groundYAt(ex,S.worldH*(.74+.025*i)),3,.13,true)});
...
function rhythm(dt){
 ...
 drawActor(playerImg,M.player,S.p.x,groundYAt(S.p.x),Math.min(5,S.combo%6),.23);if(M.opponents[0]){const ex=S.p.x+900;drawActor(oppImgs[0],M.opponents[0],ex,groundYAt(ex),Math.floor(now)%4,.22,true);}
}
