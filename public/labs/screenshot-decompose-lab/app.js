...
function bind(){
  ...
  renderIdle(els.cleanCanvas,'Build clean stage plate');
...
async function loadIncludedSample(){
  const img = new Image();
  img.onload=()=>setSourceImage(img,'Included sample screenshot');
...
function buildPlayable(){
  ...
  startLoop();
  els.runtimeStatus.textContent='Playable reconstruction running. Transparent actor proxies are overlaid on a cleaned and extended world plate.';
}
