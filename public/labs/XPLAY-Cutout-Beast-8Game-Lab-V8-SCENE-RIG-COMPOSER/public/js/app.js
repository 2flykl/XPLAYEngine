...
async function createRig(){try{const x=await j(`/api/scene-rig/${current.id}`);log(`SCENE RIG READY · ${x.rig.runtimeFamily} · world ${x.rig.world.width}×${x.rig.world.height}`);}catch(e){log(`SCENE RIG ERROR: ${e.message}`)}}
function openCalibration(){window.open(`/calibration.html?game=${encodeURIComponent(current.id)}`,'_blank')}
async function composerCheck(){try{log(`COMPOSER: checking ${current.title}…`);const x=await j(`/api/composer-check/${current.id}`,{method:'POST',headers:{'content-type':'application/json'},body:'{}'});const r=x.report;log(`COMPOSER ${r.ok?'PASS':'REVIEW'} · ${r.issues.length} issue(s) · ${r.fixes.length} correction(s)`);if(r.issues.length)log(`Issues: ${r.issues.join(' | ')}`);refresh();}catch(e){log(`COMPOSER ERROR: ${e.message}`)}}
async function genEnvironment(){
  try{
    log(`ENVIRONMENT BEAST: generating scrolling world for ${current.title}…`);
    log(`This uses 3 chained environment image calls unless cached.`);
    const x=await j(`/api/generate-environment/${current.id}`,{
      method:'POST',
      headers:{'content-type':'application/json'},
      body:JSON.stringify({force:$('#force').checked})
    });
    log(`${x.cached?'CACHE':'GENERATED'} environment · ${x.manifest.mode} · ${x.manifest.chunkCount} chunks · world ${x.manifest.worldWidth}px`);
    refresh();
  }catch(e){log(`ENVIRONMENT ERROR: ${e.message}`)}
}

async function rebuildWorld(){
  try{
    log(`Rebuilding playable with Environment Beast: ${current.title}…`);
    const x=await j(`/api/build/${current.id}`,{method:'POST',headers:{'content-type':'application/json'},body:'{}'});
    log(`WORLD BUILT ${x.runtime} · ${x.buildId}`);
    window.open(x.playUrl,'_blank');
  }catch(e){log(`REBUILD ERROR: ${e.message}`)}
}
...
$('#rigBtn').onclick=createRig;$('#calBtn').onclick=openCalibration;$('#stageBtn').onclick=genStage;$('#buildBtn').onclick=buildPlay;$('#compBtn').onclick=genComplementary;$('#envBtn').onclick=genEnvironment;$('#composerBtn').onclick=composerCheck;$('#rebuildBtn').onclick=rebuildWorld;$('#refreshBtn').onclick=refresh;$('#proofBtn').onclick=async()=>{await genStage();if(current.actors?.[0])await genSheet(current.actors[0].id);if(current.actors?.[1])await genSheet(current.actors[1].id)};
boot();
