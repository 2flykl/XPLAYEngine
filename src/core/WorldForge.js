...
function signatureAccent(dna){const P=dna.palette;return canvas(300,105,x=>{x.clearRect(0,0,300,105);x.globalCompositeOperation='lighter';const g=x.createLinearGradient(20,52,280,52);g.addColorStop(0,'rgba(255,255,255,0)');g.addColorStop(.45,P[1]);g.addColorStop(.65,P[3]);g.addColorStop(1,'rgba(255,255,255,0)');x.strokeStyle=g;x.lineWidth=12;x.beginPath();x.moveTo(18,70);x.quadraticCurveTo(120,8,282,48);x.stroke();x.lineWidth=3;x.strokeStyle=P[2];x.beginPath();x.moveTo(28,82);x.quadraticCurveTo(145,28,270,60);x.stroke();});}
...
export function forgeWorldKit(dna,artBible){
  const assets={sky:makeSky(dna),backgroundFar:makeFar(dna),backgroundMid:makeMid(dna),backgroundNear:makeNear(dna),goal:finishGate(dna),signatureJet:dna.theme==='airport'?jet(dna):signatureAccent(dna),weapon:weapon(dna),crosshair:crosshair(dna),hitfx:hitfx(dna),playerFighter:fighter(dna,0),enemyFighter:fighter(dna,1)};
  ...
  const meta={version:3,terrainKeys,propKeys,hazardKeys,collectibleKeys,enemyKeys,backgroundKeys:['sky','backgroundFar','backgroundMid','backgroundNear'],ambientSystems:dna.theme==='airport'?['moving-clouds','runway-light-pulse','signature-traffic']:['ambient-drift','scene-light-pulse','signature-accent'],counts:{terrain:terrainKeys.length,props:propKeys.length,hazards:hazardKeys.length,collectibles:collectibleKeys.length,enemies:enemyKeys.length,layers:5,ambient:3},math:{seed:dna.seed,noise:'fBm-4-octave',placement:'entropy+poisson',difficulty:'logistic-s-curve',parallax:'depth-power-law'}};
  return {assets,meta};
}
