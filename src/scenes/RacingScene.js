...
export class RacingScene extends BasePLXScene{
  ...
  create(){
    ...
    this.car=this.createFluxPlayer(480,490,{scale:.72,anim:'drive_straight'});this.player=this.car;
    ...
    this.physics.add.overlap(this.car,this.traffic,()=>{this.speed=Math.max(180,this.speed-80);this.setScore(Math.max(0,this.score-50));this.flashFlux(this.car,'crash','drive_straight');this.cameras.main.shake(100,.012);},null,this);
    this.physics.add.overlap(this.car,this.boosts,(p,b)=>{b.destroy();this.speed=Math.min(480,this.speed+65);this.flashFlux(this.car,'boost','drive_straight');this.setScore(this.score+100);},null,this);
  }
  ...
  update(_,dt){
    ...
    if(this.cursors.left.isDown||this.wasd.A.isDown){this.car.setVelocityX(-steer);this.playFlux(this.car,'steer_left');}
    if(this.cursors.right.isDown||this.wasd.D.isDown){this.car.setVelocityX(steer);this.playFlux(this.car,'steer_right');}
