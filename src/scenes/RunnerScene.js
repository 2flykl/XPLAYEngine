...
export class RunnerScene extends BasePLXScene{
  ...
  create(){
    ...
    this.player=this.createPlayerCharacter(160,430,{scale:.82,anim:'run',maxHeight:128,maxWidth:96});
    ...
    this.physics.add.overlap(this.player,this.hazards,()=>{this.setScore(Math.max(0,this.score-150));this.flashCharacter(this.player,'hurt','run');this.cameras.main.shake(120,.009)},null,this);
  ...
  update(_,dt){
    ...
    if((Phaser.Input.Keyboard.JustDown(this.space)||Phaser.Input.Keyboard.JustDown(this.cursors.up))&&this.player.body.blocked.down){this.player.setVelocityY(-535);this.flashCharacter(this.player,'jump','run');}
