...
function frameKey(v){
  ...
  if(v && typeof v==='object') {
    for(const k of ['key','texture','name','id','src']) {
...
export class BasePLXScene extends Phaser.Scene {
  constructor(key,manifest){
    ...
    this.score=0;
    this.fluxCfg=FLUX_RUNTIME[this.manifest.engine]||null;
  ...
  asset(path){
    ...
    return `${typeof this.manifest.__base==='string'?this.manifest.__base:''}${ref}`;
  }

  preload(){
    const a=this.manifest.assets||{};
    for(const [k,p] of Object.entries(sanitizeAssetMap(a.images||{}))){
      const ref=this.asset(p); if(ref)this.load.image(k,ref);
    }
    for(const [k,p] of Object.entries(sanitizeAssetMap(a.audio||{}))){
      const ref=this.asset(p); if(ref)this.load.audio(k,ref);
    }
    for(const [k,p] of Object.entries(sanitizeAssetMap(this.manifest.parallax||{}))){
      const ref=this.asset(p); if(ref)this.load.image(`parallax:${k}`,ref);
    }

    // Flux is opt-in only. Missing/deleted Flux packs must never break unrelated PLXs.
    if(this.manifest.characterPolicy?.useFlux===true) this.preloadFlux();
  }
  ...
  setupFluxAnimations(){
    if(!this.fluxCfg||this._fluxAnimationsReady||this.manifest.characterPolicy?.useFlux!==true)return;
    for(const anim of this.fluxCfg.animations){
      const key=`flux-${this.manifest.engine}-${anim}`;
      if(!this.anims.exists(key)){
        this.anims.create({
          key,
          frames:Array.from({length:15},(_,i)=>({key:`flux:${anim}:${i}`})),
          frameRate:['runner','racing'].includes(this.manifest.engine)?14:12,
          repeat:['hurt','victory','finish','jump','land','fire','reload','kick','punch','correct','wrong'].includes(anim)?0:-1
        });
      }
    }
  ...
  setupGeneratedPlayerAnimations(){
    const map=this.manifest.generatedAnimations?.player;
    if(!map||this._generatedPlayerAnimationsReady)return false;

    let created=false;
    for(const [state,frames] of Object.entries(map)){
      const list=Array.isArray(frames)?frames:[frames];
      const valid=list.map(frameKey).filter(Boolean).filter(k=>this.textures.exists(k));
      if(!valid.length)continue;
      const key=`generated-player-${state}`;
      if(!this.anims.exists(key)){
        this.anims.create({
          key,
          frames:valid.map(k=>({key:k})),
          frameRate:state==='run'?12:8,
          repeat:['idle','run','fall'].includes(state)?-1:0
        });
      }
      created=true;
    }
    this._generatedPlayerAnimationsReady=true;
    return created;
  }
  ...
  createFluxPlayer(x,y,opts={}){
    ...
    if(generated&&this.setupGeneratedPlayerAnimations()){
      const candidates=generated[opts.anim||'idle']||generated.idle||[];
      const list=Array.isArray(candidates)?candidates:[candidates];
      const first=list.map(frameKey).filter(Boolean).find(k=>this.textures.exists(k));
      if(first){
        const p=opts.physics===false?this.add.sprite(x,y,first):this.physics.add.sprite(x,y,first);
        const src=p.texture.getSourceImage(),maxH=opts.maxHeight||150,maxW=opts.maxWidth||115,sw=src?.width||128,sh=src?.height||128;
        p.setScale(Math.min(maxW/sw,maxH/sh)*(opts.scale?opts.scale/.72:1));
        if(opts.depth!=null)p.setDepth(opts.depth);
        if(opts.physics!==false&&opts.collideWorldBounds!==false)p.setCollideWorldBounds(true);
        p.__generatedPlayer=true;
        this.playFlux(p,opts.anim||'idle',false);
        return p;
      }
    }
    ...
    if(this.manifest.characterPolicy?.useFlux===true){
    ...
    // Last-resort neutral runtime avatar. Never block a whole PLX because character art is absent.
    const g=this.add.circle(x,y,28,0x24c9c5,1).setStrokeStyle(4,0xffffff,.9);
    if(opts.physics!==false)this.physics.add.existing(g);
    g.__neutralFallback=true;
    return g;
  }

  playFlux(sprite,anim,ignoreIfPlaying=true){
    if(!sprite)return;
    if(sprite.__generatedPlayer){
      const key=`generated-player-${anim}`;
      if(this.anims.exists(key)&&sprite.anims?.currentAnim?.key!==key)sprite.play(key,ignoreIfPlaying);
      return;
    }
    if(sprite.__sourceSubject||sprite.__neutralFallback||this.manifest.characterPolicy?.useFlux!==true||!this.fluxCfg?.animations.includes(anim))return;
    const key=`flux-${this.manifest.engine}-${anim}`;
  ...
  flashFlux(sprite,anim,returnAnim=this.fluxCfg?.default||'idle'){
    ...
    if(sprite.anims) sprite.once(Phaser.Animations.Events.ANIMATION_COMPLETE,()=>this.playFlux(sprite,returnAnim,false));
  }

  makeParallaxBackground({speed=0,fixed=true,axis='x',direction='left'}={}){
    const par=this.manifest.parallax||{},layers=[];
  ...
  updateParallax(dt=16,mult=1){
    ...
    const f=dt/16.666,total=this._parallaxLayers.length,speeds=this._parallaxLayers.map((_,i)=>Math.max(.12,parallaxFactor(total-1-i,total,1.55)));
    const sign={left:1,right:-1,down:-1,up:1}[this._parallaxDirection]??1;
    this._parallaxLayers.forEach((l,i)=>{const amount=this._parallaxSpeed*speeds[i]*f*mult*sign;if(this._parallaxAxis==='y')l.tilePositionY+=amount;else l.tilePositionX+=amount;});
  }
  ...
  makeHUD(){
    ...
    this.add.rectangle(480,24,956,48,0x051824,.5).setScrollFactor(0).setDepth(90);
  ...
  recordFunEvent(type,detail={}){this._funEvents.push({time:Date.now()-this._playStartedAt,type,detail});}
