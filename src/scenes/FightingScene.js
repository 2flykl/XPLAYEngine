...
export class FightingScene extends BasePLXScene{
  ...
  create(){
    ...
    this.player=this.createPlayerCharacter(245,440,{scale:1.0,anim:'stance',maxHeight:190,maxWidth:130});this.enemy=this.createOpponentCharacter(715,440,{scale:1.0,anim:'stance',maxHeight:190,maxWidth:130});this.enemy.setFlipX(true);
  ...
  animateAttack(sprite,type){this.playCharacter(sprite,type,false);const dir=sprite.flipX?-1:1;const x=sprite.x;this.tweens.killTweensOf(sprite);this.tweens.add({targets:sprite,x:x+dir*(type==='kick'?22:14),scaleX:sprite.scaleX*(type==='kick'?1.08:1.04),duration:85,yoyo:true,ease:'Quad.out',onComplete:()=>{this.playCharacter(sprite,'stance',false);this.idleTween(sprite)}});}
  attack(type='punch'){
  ...
  update(_,dt){
    ...
    this.attackCooldown=Math.max(0,this.attackCooldown-dt);this.enemyCooldown=Math.max(0,this.enemyCooldown-dt);const block=this.keys.L.isDown;this.player.setVelocityX(0);const speed=225;if(this.keys.A.isDown){this.player.setVelocityX(-speed);this.playCharacter(this.player,'walk_back');}if(this.keys.D.isDown){this.player.setVelocityX(speed);this.playCharacter(this.player,'walk_forward');}if(Phaser.Input.Keyboard.JustDown(this.keys.W)&&this.player.body.blocked.down)this.player.setVelocityY(-470);this.playCharacter(this.player,'jump',false);if(Phaser.Input.Keyboard.JustDown(this.keys.J))this.attack('punch');if(Phaser.Input.Keyboard.JustDown(this.keys.K))this.attack('kick');if(Phaser.Input.Keyboard.JustDown(this.keys.S)&&this.special>=100)this.attack('special');
    if(block){this.player.setTint(0x9ee8ff);this.playCharacter(this.player,'block');}else{this.player.clearTint();if(!this.keys.A.isDown&&!this.keys.D.isDown)this.playCharacter(this.player,'stance');}const dx=this.player.x-this.enemy.x;this.player.setFlipX(dx>0);this.enemy.setFlipX(dx<0);if(Math.abs(dx)>120)this.enemy.setVelocityX(Math.sign(dx)*125);else this.enemy.setVelocityX(0);
